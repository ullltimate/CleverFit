...
export const SettingDrawer: React.FC<SettingDrawerProps> = ({
    ...
    const payTariff = () => {
        if (tariffList)
            buyTariff({ tariffId: tariffList[0]._id, days: valueRadio })
                .unwrap()
                .then(() => {
    ...
    return (
        <Drawer
            ...
            style={{ marginTop: `${windowSize < 630 ? '85px' : '0px'}` }}
            ...
            bodyStyle={{
                padding: `0px var(--unit-${windowSize < 630 ? 8 : 32})`,
            }}
            footer={
                !tariff && (
                    <Button
        ...
        >
            ...
            {!tariff && (
                <div className='costs-tariff'>
                    <p className='costs-tariff-title'>Стоимость тарифа</p>
                    <Radio.Group
                        onChange={onChangeRadio}
                        value={valueRadio}
                        data-test-id='tariff-cost'
                    >
                        <Space direction='vertical' className='costs-tariff-radio-wrap'>
                            {tariffList &&
                                tariffList[0].periods.map((e) => (
                                    <Radio
                                        key={e.cost}
                                        value={e.days}
